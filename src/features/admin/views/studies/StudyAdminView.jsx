...
/**
 * 스터디 관리 — 목록 표와 개설 승인을 한 화면의 탭으로 묶습니다.
 *
 * 목록은 기존 표(TableView)를 그대로 쓰고, 승인 대기는 각 액션이 즉시 서버에
 * 반영되는 단건 처리라 배치저장 모델에 맞지 않아 카드형으로 둡니다
 * (일정 관리와 같은 판단입니다). 참여 신청자는 목록의 '상세' 모달이 맡습니다.
 *
...
export function StudyAdminView() {
  ...
  // 모르는 탭 키는 목록으로 되돌린다 — 없어진 '신청자' 탭을 가리키는 북마크가
  // 아무것도 그리지 않는 빈 화면으로 떨어지지 않게.
  const asked = sp.get('tab') || 'list';
  const tab = STUDY_TABS.some((t) => t.key === asked) ? asked : 'list';
  const activeTab = STUDY_TABS.find((t) => t.key === tab);
  ...
  const cancelReject = () => { setRejectId(null); setReason(''); };
  const startReject = (id) => { setRejectId(id); setReason(''); };
  ...
  return (
    <div>
      ...
      {/* 스터디를 승인하는 자리인지 사람을 승인하는 자리인지 적어 둔다. */}
      {activeTab.desc && (
      ...
      {tab === 'pending' && (pendingQ.isLoading || pendingQ.isError) && (
        <Notice>{pendingQ.isError ? '목록을 불러오지 못했습니다.' : '불러오는 중…'}</Notice>
      )}
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            {rows.map((p) => (
              <PendingCard
                ...
                onRejectStart={() => startReject(p.id)}

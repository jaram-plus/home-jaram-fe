  ...
  return (
    <ModalShell title="세미나 개설" onClose={onClose} maxWidth={540} align="top">
      <div style={{ marginTop: 22, display: 'grid', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <Input label="일시" placeholder="6월 30일 (월) 19:00" value={values.when} onChange={field('when')} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <Input label="출석 코드" placeholder="참석자에게 공지할 코드를 설정하세요" value={values.code} onChange={field('code')} />
          <Input label="발표 자료 링크" placeholder="슬라이드·문서 URL" value={values.material} onChange={field('material')} />
        </div>
      </div>
